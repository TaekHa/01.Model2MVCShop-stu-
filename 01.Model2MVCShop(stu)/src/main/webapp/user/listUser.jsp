...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	<tr>
		...
		<%if((((searchVO.getPage()-1)/indexSize)*indexSize)+1 != 1) {%>
			<a href="/listUser.do?page=<%=(((searchVO.getPage()-1)/indexSize))*indexSize%>&searchCondition=<%=searchVO.getSearchCondition() %>&searchKeyword=<%=searchVO.getSearchKeyword()%>">이전</a>
		<%} %>
		<%
		...
		%>
			...
			<a href="/listUser.do?page=<%=(((searchVO.getPage()-1)/indexSize+1)*indexSize)+1%>&searchCondition=<%=searchVO.getSearchCondition() %>&searchKeyword=<%=searchVO.getSearchKeyword()%>">다음</a>
			<%} %>
		<%}%>	
    	</td>
	</tr>
